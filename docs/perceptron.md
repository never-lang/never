...
minimum. This method is known as gradient descent. Please note that we want
to correct values in ```W``` by subtracting error. Also we want to move in gradient's opposite direction. Thus plus sign in the equation. To summarize back propagation
equation is ```W = W + xT * err * sD```.

All calculation are done for each input value at once. Matrix of input values
is given by transpose matrix T. To multiply values by their corresponding
elements matrix Hadamard multiplication is used. Value ```1``` is replaced
by matrix with all elements initialized to ```1```.
