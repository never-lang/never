...
Since all basic building blocks are described we can implement neural network.

## Forward Propagation

Supervised learning is one of algorithms used to create neural network. The
algorithm has set of inputs and set of expected results. The difference between
expected result and the generated by the network is used to change its
parameters. The algorithm may be formalized in matrix algebra.
...
were correct then we would get also proper output values.

## Back Propagation

The network needs to pass several learning cycles to change random values
```W``` set in initial phase into such which will give smallest error.
...
Of course we can keep these values in a matrix.
...
will lead to least error. For that sigmoid derivation is used. One may
easily check that first derivative of function s = 1.0 / (1.0 + exp(-x)) is
sD = s * (1 - s). First derivative (or function gradient) lets to move towards
minimum of a function. When multiplied by error we will lead towards function
minimum. The equation is ```W = W + xT * err * sD```.

As all calculation are done for each input value at once matrix algebra is used.
Matrix of input values is given by transpose matrix T. To multiply values by
their corresponding elements matrix Hadamard multiplication is used. Value
```1``` is replaced by matrix with all elements initialized to ```1```.
...
Learning cycles are executed using loop over forward and backward propagation
phases.
...
All code in Never begins in ```main``` function. 
